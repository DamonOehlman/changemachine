var debug = require('debug')('changemachine'),
    ...
    _machineCounter = 0,
    _machines = {},
    _processingStatuses = ['processing', 'fail'];
    ...
    jobmanager.addJob(machineId, {
        ...
        work: function(item, state) {
            ...
            item.once('leave', function(status) {
                machine.changeItemStatus(item, 'exited');

                debug('item left machine, marking worker as finished, job queue length = ' + job.queue.length);
...
function _makeNotifier(machine, target, opts, callback) {
    ...
    _getState(machine.storage, target, function(state) {
        var job = machine.job,
            notifier = changemate(target, opts, state);
...
Machine.prototype.changeItemStatus = function(item, newStatus) {
    ...
    // if the new status is exited, increment the exited count
    if (newStatus !== 'exited') {
        ...
        this.exitedCount = this.exitedCount + 1;
    }
};

// ### isProcessing
Machine.prototype.isProcessing = function(item) {
    var processing = false,
        queues = this.statusQueues;

    _processingStatuses.forEach(function(status) {
        processing = processing || (queues[status] && queues[status].indexOf(item) >= 0);
    });
