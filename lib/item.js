...
Item.prototype.changeStatus = function(status, opts) {
    ...
    this.machine.changeItemStatus(this, status, leaving);
    
    // emit the status event for the machine
    currentMachine.emit(status, this);
    
    // trigger the status event on the item
    this.emit.apply(item, [status].concat(Array.prototype.slice.call(arguments, 3)));
...
Item.prototype.process = function() {
    ...
    if (machine) {
        ...
        if (listeners.length && (! machine.paused)) {
            debug('processing item "' + this.id + '" machine paused: ' + this.machine.paused);
