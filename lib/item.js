...
function _changeStatus(item, status, opts) {
    ...
    debug('item "' + item.id + '" is changing status to "' + status + '", leaving = ' + leaving);
    ...
    item.machine.changeItemStatus(item, status, leaving);
...
function _processItem(item) {
    debug('processing item "' + item.id + '" machine paused: ' + item.machine.paused);
...
Object.keys(_exitCodes).forEach(function (status) {
    Item.prototype[status] = function(opts) {
        _changeStatus(this, status, opts);
        return this;
    };
});
...
Item.prototype.process = function() {
    ...
    if (machine) {
        ...
        if (listeners.length && (! machine.paused)) {
            _processItem(this);
        }
